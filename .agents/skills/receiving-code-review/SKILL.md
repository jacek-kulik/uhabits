...
description: Evaluate code review feedback on this repository and, when resolution is requested, apply supported fixes.
---
...
2. If the request is evaluation-only, report which comments are supported without editing code. When the user asks to resolve or apply the feedback, fix clear supported defects with the smallest change that addresses them, test affected behavior after each distinct fix, and check for regressions before closing the work.
3. When feedback is mistaken or would break an existing requirement, explain the evidence and propose the narrower correction. Ask for the user's judgment only when the feedback or fix is genuinely uncertain, complicated, offers materially different options, or requires a design choice; continue independent items that are clear.
